...
//This grabs the worker ID so we can indentify which worker posts which advice
...
//When the selector up top is changed, the new category of hints are displayed.
 ...
 //this function updates which type of hints are seen up top
...
//updates how many votes a piece of advice has
function ajax_updateVotes(){
  ...
  var url = "http:localhost/saveAdvice/updateVotes.php";

  if(upVotes<=-10) //if a piece of advice has ten downvotes then it is hidden from the users
  {
    url = "http:localhost/saveAdvice/updateVotesandHideHint.php";
...
//sends how many upvotes/downvotes a piece of advice has and then increases/decreases this number 
//based on if user clicks upvote or downvote button
function ajax_getandUpdateVotes(){
  var searchText = $("#hintContent").html();
  var hr = new XMLHttpRequest();
  var url = "http:localhost/saveAdvice/getVotes.php";
...
//adds hint that user writes to database so other users can see it
function ajax_post(){
  var hr = new XMLHttpRequest();
  var url = "http:localhost/saveAdvice/newDataEntry.php";
...
//this function displays advice on the top of the mturk HIT search screen based on the type
//advice(fast, good, or other) that the user chooses with the selector
//it contacts the sql database to display the advice in the category that is stored in database
function ajax_getByType(selectedType){
  var hr = new XMLHttpRequest();
  var url = "http:localhost/saveAdvice/getAdByType.php";
